...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log('=== MANUAL RENEW NOW PROCESSING ===');
    ...
    console.log('=== MANUAL RENEW NOW PROCESSING END ===');

    return NextResponse.json({ 
      success: true, 
      message: `Successfully added ${sessionsToAdd} sessions to your balance!`,
      sessionsAdded: sessionsToAdd,
    ...
    console.error('Error in manual renew_now processing:', error);
    return NextResponse.json(
      { error: 'Failed to process renew_now payment. Please contact support.' },
      { status: 500 }
    );
  }
}
