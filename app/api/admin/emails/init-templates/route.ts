  ...
  try {
    ...
    const emailTemplates = [
      ...
      },
      {
        ...
        subject: '{{reminderTitle}} - Complete Payment for Your Session',
        content: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: {{urgencyColor}};">{{reminderTitle}}</h2>
            <p>Hi {{patientName}},</p>
            <p>{{reminderMessage}}</p>
            ...
            <div style="text-align: center; margin: 30px 0;">
              <a href="{{paymentUrl}}" style="background-color: {{buttonColor}}; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 16px;">
                {{buttonText}}
              </a>
            </div>
        ...
        variables: [
          'patientName', 'therapistName', 'appointmentDate', 'amount', 'paymentUrl',
          'reminderType', 'reminderTitle', 'reminderMessage', 'buttonText', 
          'buttonColor', 'urgencyColor', 'additionalMessage', 'timeRemaining'
        ],
        isActive: true
      }
    ];
    ...
    for (const template of emailTemplates) {
      ...
      if (existing) {
        ...
        results.push({ type: template.type, action: 'updated', template: updated });
      } else {
        console.log(`Creating template ${template.type}...`);
        const created = await EmailTemplate.create(template);
        results.push({ type: template.type, action: 'created', template: created });
      }
    }

    return NextResponse.json({
      success: true,
      message: 'All email templates created/updated successfully!',
      results
    });
