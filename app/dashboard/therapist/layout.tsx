...
import CalendarConnectionAlert from "@/components/shared/CalendarConnectionAlert";
// import PayoutAlert from "@/components/shared/PayoutAlert";
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className={cn(
          ...
          <div className="relative h-full bg-white border-r border-gray-200">
            <Button
              variant="ghost"
              size="icon"
              className="absolute -right-3 top-1/2 -translate-y-1/2 h-6 w-6 rounded-full border bg-white shadow-md text-black hover:text-black"
            ...
            <div className="h-full p-4">
              <TherapistSidebar isCollapsed={isSidebarCollapsed} />
            </div>
          </div>
        </div>
      )}
      ...
      {isMobile && (
        <div className={cn(
          ...
          <div className="h-full p-4">
            <TherapistSidebar isCollapsed={false} />
          </div>
        </div>
      )}
      ...
      <div className={cn(
        ...
        <main className="container py-2 px-4 sm:px-6">
          <NotificationAlert/>
          {session.user.status === "pending" && (
            <EmailVerificationAlert userEmail={session.user.email} />
          )}
          <CalendarConnectionAlert />
          {/* {session.user.role === "therapist" && <PayoutAlert />} */}
          {children}
        </main>
      </div>
    </div>
